...
template <typename Bean, typename Enable = void> struct GetDerivedBase {
  constexpr static auto Bases = hana::make_tuple();
};

template <typename Bean>
struct GetDerivedBase<
    Bean, std::enable_if_t<describe::has_describe_bases<Bean>::value>> {
  constexpr static auto Bases = [] {
    using BaseDescriptors =
        describe::describe_bases<Bean, describe::mod_any_access>;
    using BaseTuple = mp11::mp_rename<BaseDescriptors, hana::tuple>;
    return hana::transform(BaseTuple(), [](auto base) {
      return hana::type_c<typename decltype(base)::type>;
    });
  }();
};

struct GetAllParentsFuncType {
  template <typename Beans> constexpr auto operator()(Beans &&beans) const {
    // allParents, lefts
...
struct FillImplementedMapFuncType {
  ...
  constexpr auto operator()(ImplementedMap &&map,
    ...
    auto processBean = [](auto &&bean) {
      ...
      return hana::unpack(_GetAllParents(VertexType::InList),
...
static constexpr auto ConstructBeans = [](auto &&beanVertexes) {
...
struct ChangeableBeanResolver {
  ...
  constexpr static auto DependOn = hana::to_tuple(hana::difference(
      hana::to_set(BeanResolver::DependOn), hana::to_set(IndependentTypes{})));
...
struct RemoveVertexDependencyFuncType {
  ...
  constexpr auto operator()(BeanVertexes &&vertexes,
                            IndependentBeans &&beans) const {
    ...
    auto removeDependency = [&IndependentTypes](const auto &vertex) {
      ...
      using NewBeanResolver =
          ChangeableBeanResolver<typename VertexType::NodeType,
