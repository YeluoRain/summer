...
namespace detail {
template <typename T,
          typename DerivedBases =
              describe::describe_bases<T, describe::mod_any_access>,
          typename FuncTraits =
              traits::ConstructorTraits<decltype(&T::_InjectedFactory)>>
...
template <typename T> struct BeanResolver : detail::BeanResolverImpl<T> {
  static_assert(describe::has_describe_bases<T>::value,
